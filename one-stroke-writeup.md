...
## Failed Attempt 1 — Basic File Triage

I started with the standard forensic checks.

I inspected the JPEG structure, metadata, possible appended data, and obvious embedded files.
...
- data after the JPEG EOI marker
- ZIP signatures
- embedded PNG files
- PDF signatures
- JPEG comments
...
The image was simply a valid baseline JPEG. There was no obvious appended payload or metadata trick.

So that path was closed pretty quickly.

---

## Failed Attempt 2 — Pixel LSB and Color Channels

The challenge called the image “monochrome”, so naturally I checked whether the image was actually grayscale.

Interestingly, it was stored as RGB, and the three channels were not completely identical.

There were measurable differences between:
...
That made me suspect hidden data in the chroma information.

I extracted and inspected:
...
I also tested normal LSB extraction from each channel and several bit planes.

For example, I tried ideas along the lines of:
...
or anything resembling a UUID.

The RGB differences were real, but they turned out not to be the main solution.

---

## Failed Attempt 3 — JPEG DCT Steganography

Because the file was a JPEG, I moved into the JPEG coefficient domain.

I tested several common ideas:

- DCT coefficient parity
- coefficient signs
- zero/nonzero maps
- low-magnitude coefficients
- JSteg-style extraction
- different zig-zag and component traversal orders

I even investigated Steghide-style embedding more deeply.

That was probably the point where I started overcomplicating the challenge.

I found no valid JSteg payload and no usable Steghide header.

In hindsight, the challenge was not about hiding extra data inside JPEG coefficients at all. The visible pixels themselves were the data; they were just permuted.

---

## Failed Attempt 4 — Stereogram Rabbit Hole

This was my biggest wrong turn.

While analyzing horizontal correlations in the image, I noticed an interesting similarity peak around:
...
I started suspecting that the image might contain repeated horizontal texture.

Combined with the description:

> “looking at it the wrong way”

I thought it could be an autostereogram or some sort of disparity-encoded image.

So I tried:

- shifted image differences
- local SAD matching
- local NCC matching
- StereoSGBM
- self-disparity maps
- optical flow
- repeated strip alignment
- anaglyph-style visualizations

I generated a lot of output images.

Some looked vaguely structured, which only made the rabbit hole worse.

But none of them produced readable text or a UUID.

Eventually I had to admit that the 346-pixel correlation was interesting but not sufficient evidence for a stereogram.

---

## Failed Attempt 5 — FFT, Geometry, Halftone and Component Analysis

After that, I went broad.
...
Since the image consisted of lots of tiny black fragments, I also considered whether each connected component could encode information through:
...
Again, none of these produced a reliable flag.

At this point, the analysis had become too wide. I was transforming the image in almost every imaginable way without a strong model of what the actual permutation was.
